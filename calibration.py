            ...
            for cnt in contours0:
                ...
                if area > 80000:
                    ...
                    cv.putText(img, "%d" % (center[0]), (center[0] + 20, center[1] - 40), cv.FONT_HERSHEY_SIMPLEX, 1,
                               color_red, 2)
                    if((width/2 - center[0] < 5 and width/2 - center[0] > 0) or (width/2 - center[0] > -5 and width/2 - center[0] < 0)):
                        cv.imwrite(os.path.join(path, 'test-{id}.jpg'.format(id=i)), img)
                        i+=1
                        time.sleep(1.2)

                        pub = rospy.Publisher('imgpath', String, queue_size=10)
                        rospy.init_node('imgpath_topic_publisher')
                        pub.publish(os.path.join(path, 'test-{id}.jpg'.format(id=i)))
